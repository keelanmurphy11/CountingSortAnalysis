...
/*
    In the study they compare the ARU Counting Sort algorithm to Trivial Counting Sort exactly how we implemented it.
    They also compare to quickSort, mergeSort and radixSort.
    We are not given details on the implementations of these so we decided to compare it to a standard (basic) implementation
    and also to a more powerful implementation.

    The following three sorting algorithms are basic implemtations followed by three optimised implementations

    Probably should change all to ints - would surely improve efficiency
 */
...
//Create an auxiliary array just once, this will save costly operations of constantly creating new arrays
    ...
    //-------------------------------------Additional Test: Bottom up merge Sort-------------------------------------//
...
//Could test aswell???
    ...
    //-------------------------------Bucket Sort--------------------------------//
    public static void bucketSortDec(int[] array, int exp, int k){
        int n = array.length;
        if (n == 0 || k <= 0) return;
...
//-------------------------------------Radix Sort-------------------------------------//
    public static void radixSort(int[] array, int k) {
        //sort by units, tens, hundreds etc.
        //how many times do i loop??
        int n = array.length;
        int div = k; //e.g. 10 k = num buckets
...
//LEAVE INSERTION SORT IN HERE< USED IN OPTIMISATIONS!!!!!!!!!!
